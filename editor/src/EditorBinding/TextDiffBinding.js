class TextDiffBinding {
  ...
  _transformSelectionAndUpdate = (rangeOffset, length, transformCursor) => {
    ...
    if (!editor || !editor.getModel() || !this.compoThis.state.monaco) {
      this.update();
      return;
    }
  ...
  update = (isSetup) => {
    ...
    const key = this.path && this.path[0] ? this.path[0] : 'content';
    let value = this.doc.data[key];
    if (value === undefined || value === null) value = '';

    this.compoThis.setState({ code: value }, () => {
      // Update peer cursor safely
      if (this.range && this.compoThis.state.editor && this.compoThis.state.monaco) {
        try {
          ...
          // ignore transient decoration errors
        }
      }
    });
  };
